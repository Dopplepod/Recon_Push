...
def reset_file_obj(file_obj) -> None:
    try:
        file_obj.seek(0)
    except Exception:
        try:
...
def is_csv_file(file_obj) -> bool:
    name = str(getattr(file_obj, 'filename', '') or getattr(file_obj, 'name', '') or '').lower()
    return name.endswith('.csv')


def read_csv_flexible(file_obj, **kwargs) -> pd.DataFrame:
    reset_file_obj(file_obj)
    last_err = None
    options = [
    ...
    for opt in options:
        try:
            reset_file_obj(file_obj)
            return pd.read_csv(file_obj, **opt, **kwargs)
        except Exception as exc:
            last_err = exc
    raise last_err


def detect_header_row_from_preview(preview: pd.DataFrame, required_aliases: Dict[str, str]) -> int:
    required_targets = set(required_aliases.values())
    best_idx = 0
    best_score = -1
    for idx in range(len(preview)):
